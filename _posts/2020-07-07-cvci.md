...
Pick the model with the best point estimate, as well as the one with the one SE rule
```python
...
We see that we end up picking a model way more regularization; more coefs? L1 size of coefs?

## What else might we do with this? Multiple comparisons and hierarchical models

We might think of comparing a bunch of models, in which case we need mtc

We could also model each model's CV as a hierarchical model (data point error -> fold error -> (model error | hyperparams)), maybe even do some bayesian optimization

## Endnotes

<a name="foot1">1</a>: A helpful reference, from which a number of these ideas were adapted directly, are the lecture notes for Stanford's Data Mining: 36-462/36-662 with Rob Tibshirani. [These notes](https://www.stat.cmu.edu/~ryantibs/datamining/lectures/18-val1.pdf) and [these notes](https://www.stat.cmu.edu/~ryantibs/datamining/lectures/19-val2.pdf) in particular are useful, as is section 7.10 of ESL.
...
<a name="foot4">4</a>: It is not clear to me what justifies the use of one standard error specifically, but it operationalizes the Occam-y idea that we want "the simplest model that makes good predictions". Even if it's not obvious that it's optimal, it seems like a very useful heuristic. Perhaps because this means the other CI is likely mostly inside the first?
