...
daily_trip_regression_data['after'] = (daily_trip_regression_data['date'] >= '2020-03-01').apply(int)
daily_trip_regression_data['after_trend'] = np.cumsum(daily_trip_regression_data['after'])

plt.plot(daily_trip_regression_data['date'], daily_trip_regression_data['trips'])

model = smf.ols('trips ~ trend + C(month) + C(year) + after + after_trend', daily_trip_regression_data)
result = model.fit()
